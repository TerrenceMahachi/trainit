...
class ActivitycategorysController
{
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $search .= "(`code` LIKE $escapedTerm OR `name` LIKE $escapedTerm OR `description` LIKE $escapedTerm OR `is_billable` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Activitycategory::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if ($is_billable == "" && (!$_error)) {  $_result .= "<br>Error: is_billable cannot be blank"; $_error = true; }

        if (!$_error) {
            $sql = "SELECT * FROM activitycategory WHERE name=?";
            $records = Activitycategory::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            $record = new Activitycategory();
            $record->reg_by = $_COOKIE['user'];
            
            $record->code = $code;
            $record->name = $name;
            $record->description = $description;
            $record->is_billable = $is_billable;
            $record->save();
    ...
    public function edit($recordiD)
    {
        ...
        if ($is_billable == "" && (!$_error)) {  $_result .= "<br>Error: is_billable cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM activitycategory WHERE name=? AND iD!=?";
            $records = Activitycategory::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Activitycategory::where('iD', $recordiD)[0];
