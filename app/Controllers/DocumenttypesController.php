...
class DocumenttypesController
{
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $search .= "(`code` LIKE $escapedTerm OR `name` LIKE $escapedTerm OR `description` LIKE $escapedTerm OR `requires_expiry` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Documenttype::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        $requires_expiry = $_POST["requires_expiry"];
        ...
        if ($requires_expiry == "" && (!$_error)) {  $_result .= "<br>Error: requires_expiry cannot be blank"; $_error = true; }

        if (!$_error) {
            $sql = "SELECT * FROM documenttype WHERE name=?";
            $records = Documenttype::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            ...
            $record->requires_expiry = $requires_expiry;
            $record->save();
    ...
    public function edit($recordiD)
    {
        ...
        if ($requires_expiry == "" && (!$_error)) {  $_result .= "<br>Error: requires_expiry cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM documenttype WHERE name=? AND iD!=?";
            $records = Documenttype::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Documenttype::where('iD', $recordiD)[0];
