...
class EmploymentstatussController
{
    public function index()
    {
        ...
        $pagination_data = Employmentstatus::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM employmentstatus WHERE name=?";
            $records = Employmentstatus::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
    ...
    public function edit($recordiD)
    {
        ...
        if ($description == "" && (!$_error)) {  $_result .= "<br>Error: description cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM employmentstatus WHERE name=? AND iD!=?";
            $records = Employmentstatus::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Employmentstatus::where('iD', $recordiD)[0];
