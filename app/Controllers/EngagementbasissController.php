...
class EngagementbasissController
{
    public function index()
    {
        ...
        $pagination_data = Engagementbasis::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM engagementbasis WHERE name=?";
            $records = Engagementbasis::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
    ...
    public function edit($recordiD)
    {
        ...
        if ($name == "" && (!$_error)) {  $_result .= "<br>Error: name cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM engagementbasis WHERE name=? AND iD!=?";
            $records = Engagementbasis::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Engagementbasis::where('iD', $recordiD)[0];
