...
class PayrollitemtypesController
{
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $search .= "(`code` LIKE $escapedTerm OR `name` LIKE $escapedTerm OR `description` LIKE $escapedTerm OR `is_deduction` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Payrollitemtype::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        $is_deduction = $_POST["is_deduction"];
        ...
        if ($is_deduction == "" && (!$_error)) {  $_result .= "<br>Error: is_deduction cannot be blank"; $_error = true; }

        if (!$_error) {
            $sql = "SELECT * FROM payrollitemtype WHERE name=?";
            $records = Payrollitemtype::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            $record = new Payrollitemtype();
            $record->reg_by = $_COOKIE['user'];
            
            $record->code = $code;
            $record->name = $name;
            $record->description = $description;
            $record->is_deduction = $is_deduction;
            $record->save();
    ...
    public function edit($recordiD)
    {
        ...
        if ($is_deduction == "" && (!$_error)) {  $_result .= "<br>Error: is_deduction cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM payrollitemtype WHERE name=? AND iD!=?";
            $records = Payrollitemtype::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Payrollitemtype::where('iD', $recordiD)[0];
