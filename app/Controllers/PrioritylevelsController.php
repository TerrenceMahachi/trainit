...
class PrioritylevelsController
{
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $search .= "(`code` LIKE $escapedTerm OR `name` LIKE $escapedTerm OR `description` LIKE $escapedTerm OR `response_minutes` LIKE $escapedTerm OR `resolution_hours` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Prioritylevel::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM prioritylevel WHERE name=?";
            $records = Prioritylevel::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
    ...
    public function edit($recordiD)
    {
        ...
        if ($resolution_hours == "" && (!$_error)) {  $_result .= "<br>Error: resolution_hours cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM prioritylevel WHERE name=? AND iD!=?";
            $records = Prioritylevel::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Prioritylevel::where('iD', $recordiD)[0];
