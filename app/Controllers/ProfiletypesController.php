...
class ProfiletypesController
{
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $search .= "(`code` LIKE $escapedTerm OR `name` LIKE $escapedTerm OR `description` LIKE $escapedTerm OR `icon` LIKE $escapedTerm OR `requires_approval` LIKE $escapedTerm OR `sort_order` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Profiletype::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if ($icon == "" && (!$_error)) {  $_result .= "<br>Error: icon cannot be blank"; $_error = true; }
        if ($requires_approval == "" && (!$_error)) {  $_result .= "<br>Error: requires_approval cannot be blank"; $_error = true; }
        if ($sort_order == "" && (!$_error)) {  $_result .= "<br>Error: sort_order cannot be blank"; $_error = true; }

        if (!$_error) {
            $sql = "SELECT * FROM profiletype WHERE name=?";
            $records = Profiletype::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
    ...
    public function edit($recordiD)
    {
        ...
        if ($sort_order == "" && (!$_error)) {  $_result .= "<br>Error: sort_order cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM profiletype WHERE name=? AND iD!=?";
            $records = Profiletype::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Profiletype::where('iD', $recordiD)[0];
