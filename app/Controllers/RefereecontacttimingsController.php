...
class RefereecontacttimingsController
{
    public function index()
    {
        ...
        $pagination_data = Refereecontacttiming::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM refereecontacttiming WHERE name=?";
            $records = Refereecontacttiming::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            $record = new Refereecontacttiming();
    ...
    public function edit($recordiD)
    {
        ...
        if ($name == "" && (!$_error)) {  $_result .= "<br>Error: name cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM refereecontacttiming WHERE name=? AND iD!=?";
            $records = Refereecontacttiming::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Refereecontacttiming::where('iD', $recordiD)[0];
