...
class RefereeverificationstatussController
{
    public function index()
    {
        ...
        if (isset($_POST['search'])) {
            ...
            $search .= "(`code` LIKE $escapedTerm OR `name` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Refereeverificationstatus::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM refereeverificationstatus WHERE name=?";
            $records = Refereeverificationstatus::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            $record = new Refereeverificationstatus();
    ...
    public function edit($recordiD)
    {
        ...
        if ($name == "" && (!$_error)) {  $_result .= "<br>Error: name cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM refereeverificationstatus WHERE name=? AND iD!=?";
            $records = Refereeverificationstatus::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Refereeverificationstatus::where('iD', $recordiD)[0];
