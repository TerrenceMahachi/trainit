...
class StaffleaveattachmentsController
{
...
if (isset($_POST['staffleave']) && $_POST['staffleave'] != '') {
    ...
    $search .= ' staffleave=' . $_POST['staffleave'];
        ...
        if (isset($_POST['search'])) {
            ...
            if ($search != "") {  $search .= " AND ";  }
            $search .= "(`file_path` LIKE $escapedTerm OR `file_size` LIKE $escapedTerm OR `mime_type` LIKE $escapedTerm)";
        }
        ...
        $pagination_data = Staffleaveattachment::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM staffleaveattachment WHERE name=?";
            $records = Staffleaveattachment::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            $record = new Staffleaveattachment();
    ...
    public function edit($recordiD)
    {
        ...
        if ($mime_type == "" && (!$_error)) {  $_result .= "<br>Error: mime_type cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM staffleaveattachment WHERE name=? AND iD!=?";
            $records = Staffleaveattachment::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Staffleaveattachment::where('iD', $recordiD)[0];
