...
class StatutoryreturnfilesController
{
...
if (isset($_POST['statutoryreturn']) && $_POST['statutoryreturn'] != '') {
    if ($search != '') {  $search .= ' AND '; }
    $search .= ' statutoryreturn=' . $_POST['statutoryreturn'];
}
        ...
        $pagination_data = Statutoryreturnfile::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        $statutoryreturn = $_POST["statutoryreturn"];
        ...
        if ($statutoryreturn == "" && (!$_error)) {  $_result .= "<br>Error: Please provide a valid value for fk_statutoryreturn "; $_error = true; }
        ...
        if (!$_error) {
            $sql = "SELECT * FROM statutoryreturnfile WHERE name=?";
            $records = Statutoryreturnfile::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
         ...
         if (!$_error) {
            $record = new Statutoryreturnfile();
            $record->reg_by = $_COOKIE['user'];
            
            $record->statutoryreturn = $statutoryreturn;
    ...
    public function edit($recordiD)
    {
        ...
        if ($mime_type == "" && (!$_error)) {  $_result .= "<br>Error: mime_type cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM statutoryreturnfile WHERE name=? AND iD!=?";
            $records = Statutoryreturnfile::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Statutoryreturnfile::where('iD', $recordiD)[0];
