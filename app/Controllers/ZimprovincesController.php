...
class ZimprovincesController
{
    public function index()
    {
        ...
        $pagination_data = Zimprovince::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM zimprovince WHERE name=?";
            $records = Zimprovince::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
    ...
    public function edit($recordiD)
    {
        ...
        if ($sort_order == "" && (!$_error)) {  $_result .= "<br>Error: sort_order cannot be blank"; $_error = true; }
        if (!$_error) {
            $sql = "SELECT * FROM zimprovince WHERE name=? AND iD!=?";
            $records = Zimprovince::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = Zimprovince::where('iD', $recordiD)[0];
