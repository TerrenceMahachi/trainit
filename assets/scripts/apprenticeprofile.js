const displayColumns = [
    ...
    { key: 'apprenticestatus', label: 'apprenticestatus' },
    { key: 'institution_name', label: 'institution name' },
    { key: 'degree_programme', label: 'degree programme' },
    { key: 'study_level', label: 'study level' },
    { key: 'student_reg_number', label: 'student reg number' },
    { key: 'expected_completion_date', label: 'expected completion date' },
    { key: 'is_wrl_attachment', label: 'is wrl attachment' },
    { key: 'wrl_start_date', label: 'wrl start date' },
    { key: 'wrl_end_date', label: 'wrl end date' },
    { key: 'wrl_duration_months', label: 'wrl duration months' },
    { key: 'wrl_coordinator_name', label: 'wrl coordinator name' },
    { key: 'wrl_coordinator_email', label: 'wrl coordinator email' },
    { key: 'wrl_coordinator_phone', label: 'wrl coordinator phone' },
    { key: 'requires_placement_letter', label: 'requires placement letter' },
    { key: 'requires_host_mou', label: 'requires host mou' },
    { key: 'requires_logbook_visits', label: 'requires logbook visits' },
    { key: 'requires_host_insurance', label: 'requires host insurance' },
    { key: 'min_stipend_required', label: 'min stipend required' },
    { key: 'engagementmodel', label: 'engagementmodel' },
    { key: 'worklocationpreference', label: 'worklocationpreference' },
    { key: 'proof_of_registration_doc', label: 'proof of registration doc' },
    { key: 'transcript_doc', label: 'transcript doc' },
    { key: 'current_average_grade', label: 'current average grade' },
];
...
function loadData() {
    ...
    var n_apprenticestatus = $('.search').find('[name=apprenticestatus]').val();
    var n_engagementmodel = $('.search').find('[name=engagementmodel]').val();
    var n_worklocationpreference = $('.search').find('[name=worklocationpreference]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-apprenticeprofile-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            apprenticestatus: n_apprenticestatus,
            engagementmodel: n_engagementmodel,
            worklocationpreference: n_worklocationpreference
        },
