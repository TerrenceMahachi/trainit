const displayColumns = [
    { key: 'workassignment', label: 'workassignment' },
    { key: 'supervisor_user', label: 'supervisor user' },
    { key: 'supervision_notes', label: 'supervision notes' },
];
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_workassignment = $('.search').find('[name=workassignment]').val();
    var n_supervisor_user = $('.search').find('[name=supervisor_user]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-assignmentsupervisor-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            workassignment: n_workassignment,
            supervisor_user: n_supervisor_user
        },
