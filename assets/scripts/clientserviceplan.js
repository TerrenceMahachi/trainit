const displayColumns = [
    { key: 'clientorganization', label: 'clientorganization' },
    { key: 'serviceoffering', label: 'serviceoffering' },
    { key: 'plan_name', label: 'plan name' },
    { key: 'currency', label: 'currency' },
    { key: 'monthly_fee', label: 'monthly fee' },
    { key: 'included_hours', label: 'included hours' },
    { key: 'associate_rate', label: 'associate rate' },
    { key: 'apprentice_rate', label: 'apprentice rate' },
    { key: 'billing_cycle_day', label: 'billing cycle day' },
    { key: 'service_manager', label: 'service manager' },
    { key: 'billing_owner', label: 'billing owner' },
    { key: 'excesspolicy', label: 'excesspolicy' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_clientorganization = $('.search').find('[name=clientorganization]').val();
    var n_serviceoffering = $('.search').find('[name=serviceoffering]').val();
    var n_service_manager = $('.search').find('[name=service_manager]').val();
    var n_billing_owner = $('.search').find('[name=billing_owner]').val();
    var n_excesspolicy = $('.search').find('[name=excesspolicy]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-clientserviceplan-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            clientorganization: n_clientorganization,
            serviceoffering: n_serviceoffering,
            service_manager: n_service_manager,
            billing_owner: n_billing_owner,
            excesspolicy: n_excesspolicy
        },
