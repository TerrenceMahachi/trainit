const displayColumns = [
    { key: 'staffdocument', label: 'staffdocument' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_staffdocument = $('.search').find('[name=staffdocument]').val();
    var n_verificationstatus = $('.search').find('[name=verificationstatus]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-documentverification-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            staffdocument: n_staffdocument,
            verificationstatus: n_verificationstatus
        },
