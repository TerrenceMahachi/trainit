const displayColumns = [
    { key: 'payrollperiod', label: 'payrollperiod' },
    { key: 'payperiodstatus', label: 'payperiodstatus' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_payrollperiod = $('.search').find('[name=payrollperiod]').val();
    var n_payperiodstatus = $('.search').find('[name=payperiodstatus]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-payrollperiodapproval-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            payrollperiod: n_payrollperiod,
            payperiodstatus: n_payperiodstatus
        },
