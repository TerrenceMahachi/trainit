const displayColumns = [
    ...
    { key: 'payrollitemtype', label: 'payrollitemtype' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_payslip = $('.search').find('[name=payslip]').val();
    var n_payrollitemtype = $('.search').find('[name=payrollitemtype]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-payslipitem-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            page_size: ps,
            payslip: n_payslip,
            payrollitemtype: n_payrollitemtype
        },
