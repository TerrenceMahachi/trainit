...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_userprofile = $('.search').find('[name=userprofile]').val();
    var n_from_status = $('.search').find('[name=from_status]').val();
    var n_to_status = $('.search').find('[name=to_status]').val();
    var n_performed_by = $('.search').find('[name=performed_by]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-profilerequestaudit-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            from_status: n_from_status,
            to_status: n_to_status,
            performed_by: n_performed_by
        },
