const displayColumns = [
    ...
    { key: 'vettingrecommendation', label: 'vettingrecommendation' },
    { key: 'eligibility_gate_passed', label: 'eligibility gate passed' },
    { key: 'technical_fit_score', label: 'technical fit score' },
    { key: 'evidence_score', label: 'evidence score' },
    { key: 'judgement_score', label: 'judgement score' },
    { key: 'availability_score', label: 'availability score' },
    { key: 'motivation_score', label: 'motivation score' },
    { key: 'total_score', label: 'total score' },
    { key: 'automated_red_flags', label: 'automated red flags' },
    { key: 'interview_notes', label: 'interview notes' },
    { key: 'technical_test_result', label: 'technical test result' },
    { key: 'vetted_at', label: 'vetted at' },
];
...
function loadData() {
    ...
    var n_reviewer = $('.search').find('[name=reviewer]').val();
    var n_vettingrecommendation = $('.search').find('[name=vettingrecommendation]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-rosterassessment-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            reviewer: n_reviewer,
            vettingrecommendation: n_vettingrecommendation
        },
