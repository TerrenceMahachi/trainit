const displayColumns = [
    ...
    { key: 'skillitem', label: 'skillitem' },
    { key: 'proficiencylevel', label: 'proficiencylevel' },
];
...
function loadData() {
    ...
    var n_skillitem = $('.search').find('[name=skillitem]').val();
    var n_proficiencylevel = $('.search').find('[name=proficiencylevel]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-rosterskill-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            skillitem: n_skillitem,
            proficiencylevel: n_proficiencylevel
        },
