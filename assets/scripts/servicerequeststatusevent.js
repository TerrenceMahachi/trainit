const displayColumns = [
    ...
    { key: 'servicerequeststatus', label: 'servicerequeststatus' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_servicerequest = $('.search').find('[name=servicerequest]').val();
    var n_servicerequeststatus = $('.search').find('[name=servicerequeststatus]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-servicerequeststatusevent-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            servicerequest: n_servicerequest,
            servicerequeststatus: n_servicerequeststatus
        },
