const displayColumns = [
    { key: 'staffleave', label: 'staffleave' },
    { key: 'leavestatus', label: 'leavestatus' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_staffleave = $('.search').find('[name=staffleave]').val();
    var n_leavestatus = $('.search').find('[name=leavestatus]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-staffleaveapproval-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            staffleave: n_staffleave,
            leavestatus: n_leavestatus
        },
