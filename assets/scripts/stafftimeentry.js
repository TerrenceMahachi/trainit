const displayColumns = [
    { key: 'staffprofile', label: 'staffprofile' },
    { key: 'activitycategory', label: 'activitycategory' },
...
function loadData() {
    $('#results').html('<div class="text-center text-muted py-4"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</div>');
        var n_staffprofile = $('.search').find('[name=staffprofile]').val();
    var n_activitycategory = $('.search').find('[name=activitycategory]').val();

    const search = $('input[name="search"]').val();
    const ps = pageState.get('page_size', '10');
    const ob = $('#order_filter').val();
    var uri = site + "/get-stafftimeentry-records";
    ...
    $.ajax({
        ...
        data: {
            ...
            order_by: ob,
            page_size: ps,
            staffprofile: n_staffprofile,
            activitycategory: n_activitycategory
        },
