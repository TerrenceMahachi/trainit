...
class {{MODEL_NAME}}sController
{
    public function index()
    {
        ...
        $pagination_data = {{MODEL_NAME}}::page($selected_page, $page_size, $search, $order_by);
        $data = [
            "order_by" => $order_by,
            "status" => "success",
            "response_code" => "002",
    ...
    { 
        ...
        if (!$_error) {
            $sql = "SELECT * FROM {{MODEL_NAME_LOWERCASE}} WHERE name=?";
            $records = {{MODEL_NAME}}::findByQuery($sql, [$name]);
            if (count($records) > 0) {
                $_error = true;
    ...
    public function edit($recordiD)
    {
        ...
        {{COLUMNS_VALIDATION}}
        if (!$_error) {
            $sql = "SELECT * FROM {{MODEL_NAME_LOWERCASE}} WHERE name=? AND iD!=?";
            $records = {{MODEL_NAME}}::findByQuery($sql, [$name, $recordiD]);
            if (count($records) > 0) {
                $_error = true;
                $_result .= "Error: Record name already used, try a different name";
            }
         ...
         if (!$_error) {
            $record = {{MODEL_NAME}}::where('iD', $recordiD)[0];
            {{COLUMNS}}
