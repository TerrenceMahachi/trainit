...
use App\Controllers\ApplicationtracksController;
use App\Helpers\ViewAccess;
use App\Models\Applicationtrack;

$uri = 'applicationtracks';
$table = 'applicationtrack';
$options['view'] = 'applicationtracks';
$options['title'] = 'Applicationtrack';
$options['MainController'] = new ApplicationtracksController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Applicationtrack::where('iD', $recordiD);
    if (count($records) === 0) {
