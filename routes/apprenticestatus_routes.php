...
use App\Controllers\ApprenticestatussController;
use App\Helpers\ViewAccess;
use App\Models\Apprenticestatus;

$uri = 'apprenticestatuss';
$table = 'apprenticestatus';
$options['view'] = 'apprenticestatuss';
$options['title'] = 'Apprenticestatus';
$options['MainController'] = new ApprenticestatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Apprenticestatus::where('iD', $recordiD);
    if (count($records) === 0) {
