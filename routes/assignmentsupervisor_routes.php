...
use App\Controllers\AssignmentsupervisorsController;
use App\Helpers\ViewAccess;
use App\Models\Assignmentsupervisor;

$uri = 'assignmentsupervisors';
$table = 'assignmentsupervisor';
$options['view'] = 'assignmentsupervisors';
$options['title'] = 'Assignmentsupervisor';
$options['MainController'] = new AssignmentsupervisorsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Assignmentsupervisor::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'workassignment' => $record->workassignment()->name,
         'supervisor_user' => $record->supervisor_user()->name,
         'supervision_notes' => $record->supervision_notes
        ];
    }, $ds);
