...
use App\Controllers\ClientmemberrolesController;
use App\Helpers\ViewAccess;
use App\Models\Clientmemberrole;

$uri = 'clientmemberroles';
$table = 'clientmemberrole';
$options['view'] = 'clientmemberroles';
$options['title'] = 'Clientmemberrole';
$options['MainController'] = new ClientmemberrolesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Clientmemberrole::where('iD', $recordiD);
    if (count($records) === 0) {
