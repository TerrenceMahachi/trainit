...
use App\Controllers\ClientmembershipsController;
use App\Helpers\ViewAccess;
use App\Models\Clientmembership;

$uri = 'clientmemberships';
$table = 'clientmembership';
$options['view'] = 'clientmemberships';
$options['title'] = 'Clientmembership';
$options['MainController'] = new ClientmembershipsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Clientmembership::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'clientmemberrole' => $record->clientmemberrole()->name
        ];
    }, $ds);
