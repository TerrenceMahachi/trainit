...
use App\Controllers\ClientorganizationsController;
use App\Helpers\ViewAccess;
use App\Models\Clientorganization;

$uri = 'clientorganizations';
$table = 'clientorganization';
$options['view'] = 'clientorganizations';
$options['title'] = 'Clientorganization';
$options['MainController'] = new ClientorganizationsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Clientorganization::where('iD', $recordiD);
    if (count($records) === 0) {
