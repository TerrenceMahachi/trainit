...
use App\Controllers\ClientserviceplansController;
use App\Helpers\ViewAccess;
use App\Models\Clientserviceplan;

$uri = 'clientserviceplans';
$table = 'clientserviceplan';
$options['view'] = 'clientserviceplans';
$options['title'] = 'Clientserviceplan';
$options['MainController'] = new ClientserviceplansController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Clientserviceplan::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'serviceoffering' => $record->serviceoffering()->name,
         'plan_name' => $record->plan_name,
         'currency' => $record->currency,
         'monthly_fee' => $record->monthly_fee,
         'included_hours' => $record->included_hours,
         'associate_rate' => $record->associate_rate,
         'apprentice_rate' => $record->apprentice_rate,
         'billing_cycle_day' => $record->billing_cycle_day,
         'service_manager' => $record->service_manager()->name,
         'billing_owner' => $record->billing_owner()->name,
         'excesspolicy' => $record->excesspolicy()->name,
