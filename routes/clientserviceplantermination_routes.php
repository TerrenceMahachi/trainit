...
use App\Controllers\ClientserviceplanterminationsController;
use App\Helpers\ViewAccess;
use App\Models\Clientserviceplantermination;

$uri = 'clientserviceplanterminations';
$table = 'clientserviceplantermination';
$options['view'] = 'clientserviceplanterminations';
$options['title'] = 'Clientserviceplantermination';
$options['MainController'] = new ClientserviceplanterminationsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Clientserviceplantermination::where('iD', $recordiD);
    if (count($records) === 0) {
