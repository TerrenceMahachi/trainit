...
$uri = 'documenttypes';
$table = 'documenttype';
$options['view'] = 'documenttypes';
$options['title'] = 'Documenttype';
$options['MainController'] = new DocumenttypesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Documenttype::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'requires_expiry' => $record->requires_expiry
        ];
    }, $ds);
