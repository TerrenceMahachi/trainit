...
use App\Controllers\DocumentvaliditysController;
use App\Helpers\ViewAccess;
use App\Models\Documentvalidity;

$uri = 'documentvaliditys';
$table = 'documentvalidity';
$options['view'] = 'documentvaliditys';
$options['title'] = 'Documentvalidity';
$options['MainController'] = new DocumentvaliditysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Documentvalidity::where('iD', $recordiD);
    if (count($records) === 0) {
