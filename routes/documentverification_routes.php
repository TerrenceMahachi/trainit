...
use App\Controllers\DocumentverificationsController;
use App\Helpers\ViewAccess;
use App\Models\Documentverification;

$uri = 'documentverifications';
$table = 'documentverification';
$options['view'] = 'documentverifications';
$options['title'] = 'Documentverification';
$options['MainController'] = new DocumentverificationsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Documentverification::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'staffdocument' => $record->staffdocument()->name,
         'verificationstatus' => $record->verificationstatus()->name,
         'notes' => $record->notes
        ];
    }, $ds);
