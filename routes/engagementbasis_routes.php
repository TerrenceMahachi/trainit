...
use App\Controllers\EngagementbasissController;
use App\Helpers\ViewAccess;
use App\Models\Engagementbasis;

$uri = 'engagementbasiss';
$table = 'engagementbasis';
$options['view'] = 'engagementbasiss';
$options['title'] = 'Engagementbasis';
$options['MainController'] = new EngagementbasissController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Engagementbasis::where('iD', $recordiD);
    if (count($records) === 0) {
