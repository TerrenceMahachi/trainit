...
use App\Controllers\EngagementmodelsController;
use App\Helpers\ViewAccess;
use App\Models\Engagementmodel;

$uri = 'engagementmodels';
$table = 'engagementmodel';
$options['view'] = 'engagementmodels';
$options['title'] = 'Engagementmodel';
$options['MainController'] = new EngagementmodelsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Engagementmodel::where('iD', $recordiD);
    if (count($records) === 0) {
