...
use App\Controllers\ExcesspolicysController;
use App\Helpers\ViewAccess;
use App\Models\Excesspolicy;

$uri = 'excesspolicys';
$table = 'excesspolicy';
$options['view'] = 'excesspolicys';
$options['title'] = 'Excesspolicy';
$options['MainController'] = new ExcesspolicysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Excesspolicy::where('iD', $recordiD);
    if (count($records) === 0) {
