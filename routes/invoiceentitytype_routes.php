...
use App\Controllers\InvoiceentitytypesController;
use App\Helpers\ViewAccess;
use App\Models\Invoiceentitytype;

$uri = 'invoiceentitytypes';
$table = 'invoiceentitytype';
$options['view'] = 'invoiceentitytypes';
$options['title'] = 'Invoiceentitytype';
$options['MainController'] = new InvoiceentitytypesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Invoiceentitytype::where('iD', $recordiD);
    if (count($records) === 0) {
