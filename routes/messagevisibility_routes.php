...
use App\Controllers\MessagevisibilitysController;
use App\Helpers\ViewAccess;
use App\Models\Messagevisibility;

$uri = 'messagevisibilitys';
$table = 'messagevisibility';
$options['view'] = 'messagevisibilitys';
$options['title'] = 'Messagevisibility';
$options['MainController'] = new MessagevisibilitysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Messagevisibility::where('iD', $recordiD);
    if (count($records) === 0) {
