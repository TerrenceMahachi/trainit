...
use App\Controllers\PayperiodstatussController;
use App\Helpers\ViewAccess;
use App\Models\Payperiodstatus;

$uri = 'payperiodstatuss';
$table = 'payperiodstatus';
$options['view'] = 'payperiodstatuss';
$options['title'] = 'Payperiodstatus';
$options['MainController'] = new PayperiodstatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Payperiodstatus::where('iD', $recordiD);
    if (count($records) === 0) {
