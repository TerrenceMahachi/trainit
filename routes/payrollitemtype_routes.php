...
use App\Controllers\PayrollitemtypesController;
use App\Helpers\ViewAccess;
use App\Models\Payrollitemtype;

$uri = 'payrollitemtypes';
$table = 'payrollitemtype';
$options['view'] = 'payrollitemtypes';
$options['title'] = 'Payrollitemtype';
$options['MainController'] = new PayrollitemtypesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Payrollitemtype::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'is_deduction' => $record->is_deduction
        ];
    }, $ds);
