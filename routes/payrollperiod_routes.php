...
use App\Controllers\PayrollperiodsController;
use App\Helpers\ViewAccess;
use App\Models\Payrollperiod;

$uri = 'payrollperiods';
$table = 'payrollperiod';
$options['view'] = 'payrollperiods';
$options['title'] = 'Payrollperiod';
$options['MainController'] = new PayrollperiodsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Payrollperiod::where('iD', $recordiD);
    if (count($records) === 0) {
