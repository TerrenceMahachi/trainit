...
use App\Controllers\PayrollperiodapprovalsController;
use App\Helpers\ViewAccess;
use App\Models\Payrollperiodapproval;

$uri = 'payrollperiodapprovals';
$table = 'payrollperiodapproval';
$options['view'] = 'payrollperiodapprovals';
$options['title'] = 'Payrollperiodapproval';
$options['MainController'] = new PayrollperiodapprovalsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Payrollperiodapproval::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'payrollperiod' => $record->payrollperiod()->name,
         'payperiodstatus' => $record->payperiodstatus()->name,
         'notes' => $record->notes
        ];
    }, $ds);
