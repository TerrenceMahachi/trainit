...
use App\Controllers\PayslipitemsController;
use App\Helpers\ViewAccess;
use App\Models\Payslipitem;

$uri = 'payslipitems';
$table = 'payslipitem';
$options['view'] = 'payslipitems';
$options['title'] = 'Payslipitem';
$options['MainController'] = new PayslipitemsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Payslipitem::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'payrollitemtype' => $record->payrollitemtype()->name,
