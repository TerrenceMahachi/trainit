...
use App\Controllers\ProfessionalbodysController;
use App\Helpers\ViewAccess;
use App\Models\Professionalbody;

$uri = 'professionalbodys';
$table = 'professionalbody';
$options['view'] = 'professionalbodys';
$options['title'] = 'Professionalbody';
$options['MainController'] = new ProfessionalbodysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Professionalbody::where('iD', $recordiD);
    if (count($records) === 0) {
