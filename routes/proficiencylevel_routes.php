...
use App\Controllers\ProficiencylevelsController;
use App\Helpers\ViewAccess;
use App\Models\Proficiencylevel;

$uri = 'proficiencylevels';
$table = 'proficiencylevel';
$options['view'] = 'proficiencylevels';
$options['title'] = 'Proficiencylevel';
$options['MainController'] = new ProficiencylevelsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Proficiencylevel::where('iD', $recordiD);
    if (count($records) === 0) {
