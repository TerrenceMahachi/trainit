...
use App\Controllers\ProfilerequestauditsController;
use App\Helpers\ViewAccess;
use App\Models\Profilerequestaudit;

$uri = 'profilerequestaudits';
$table = 'profilerequestaudit';
$options['view'] = 'profilerequestaudits';
$options['title'] = 'Profilerequestaudit';
$options['MainController'] = new ProfilerequestauditsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Profilerequestaudit::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'userprofile' => $record->userprofile()->name,
         'action' => $record->action,
         'from_status' => $record->from_status()->name,
         'to_status' => $record->to_status()->name,
         'performed_by' => $record->performed_by()->name,
         'notes' => $record->notes
        ];
    }, $ds);
