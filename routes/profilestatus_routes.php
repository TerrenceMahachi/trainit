...
use App\Controllers\ProfilestatussController;
use App\Helpers\ViewAccess;
use App\Models\Profilestatus;

$uri = 'profilestatuss';
$table = 'profilestatus';
$options['view'] = 'profilestatuss';
$options['title'] = 'Profilestatus';
$options['MainController'] = new ProfilestatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Profilestatus::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'can_access_portal' => $record->can_access_portal
        ];
    }, $ds);
