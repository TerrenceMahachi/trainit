...
use App\Controllers\QualificationstatussController;
use App\Helpers\ViewAccess;
use App\Models\Qualificationstatus;

$uri = 'qualificationstatuss';
$table = 'qualificationstatus';
$options['view'] = 'qualificationstatuss';
$options['title'] = 'Qualificationstatus';
$options['MainController'] = new QualificationstatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Qualificationstatus::where('iD', $recordiD);
    if (count($records) === 0) {
