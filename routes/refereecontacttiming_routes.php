...
use App\Controllers\RefereecontacttimingsController;
use App\Helpers\ViewAccess;
use App\Models\Refereecontacttiming;

$uri = 'refereecontacttimings';
$table = 'refereecontacttiming';
$options['view'] = 'refereecontacttimings';
$options['title'] = 'Refereecontacttiming';
$options['MainController'] = new RefereecontacttimingsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Refereecontacttiming::where('iD', $recordiD);
    if (count($records) === 0) {
