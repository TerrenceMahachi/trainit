...
use App\Controllers\RefereeverificationstatussController;
use App\Helpers\ViewAccess;
use App\Models\Refereeverificationstatus;

$uri = 'refereeverificationstatuss';
$table = 'refereeverificationstatus';
$options['view'] = 'refereeverificationstatuss';
$options['title'] = 'Refereeverificationstatus';
$options['MainController'] = new RefereeverificationstatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Refereeverificationstatus::where('iD', $recordiD);
    if (count($records) === 0) {
