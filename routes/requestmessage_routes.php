...
use App\Controllers\RequestmessagesController;
use App\Helpers\ViewAccess;
use App\Models\Requestmessage;

$uri = 'requestmessages';
$table = 'requestmessage';
$options['view'] = 'requestmessages';
$options['title'] = 'Requestmessage';
$options['MainController'] = new RequestmessagesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Requestmessage::where('iD', $recordiD);
    if (count($records) === 0) {
