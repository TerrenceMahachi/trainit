...
use App\Controllers\RequestmessageattachmentsController;
use App\Helpers\ViewAccess;
use App\Models\Requestmessageattachment;

$uri = 'requestmessageattachments';
$table = 'requestmessageattachment';
$options['view'] = 'requestmessageattachments';
$options['title'] = 'Requestmessageattachment';
$options['MainController'] = new RequestmessageattachmentsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Requestmessageattachment::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'requestmessage' => $record->requestmessage()->name,
