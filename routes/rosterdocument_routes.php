...
use App\Controllers\RosterdocumentsController;
use App\Helpers\ViewAccess;
use App\Models\Rosterdocument;

$uri = 'rosterdocuments';
$table = 'rosterdocument';
$options['view'] = 'rosterdocuments';
$options['title'] = 'Rosterdocument';
$options['MainController'] = new RosterdocumentsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Rosterdocument::where('iD', $recordiD);
    if (count($records) === 0) {
