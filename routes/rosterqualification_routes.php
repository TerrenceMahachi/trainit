...
use App\Controllers\RosterqualificationsController;
use App\Helpers\ViewAccess;
use App\Models\Rosterqualification;

$uri = 'rosterqualifications';
$table = 'rosterqualification';
$options['view'] = 'rosterqualifications';
$options['title'] = 'Rosterqualification';
$options['MainController'] = new RosterqualificationsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Rosterqualification::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'qualificationtype' => $record->qualificationtype()->name,
         'professionalbody' => $record->professionalbody()->name,
         'qualificationstatus' => $record->qualificationstatus()->name,
         'title' => $record->title,
         'institution_name' => $record->institution_name,
         'field_of_study' => $record->field_of_study,
         'date_obtained' => $record->date_obtained,
         'expiry_date' => $record->expiry_date,
         'certificate_doc' => $record->certificate_doc
        ];
    }, $ds);
