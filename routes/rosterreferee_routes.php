...
use App\Controllers\RosterrefereesController;
use App\Helpers\ViewAccess;
use App\Models\Rosterreferee;

$uri = 'rosterreferees';
$table = 'rosterreferee';
$options['view'] = 'rosterreferees';
$options['title'] = 'Rosterreferee';
$options['MainController'] = new RosterrefereesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Rosterreferee::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'refereecontacttiming' => $record->refereecontacttiming()->name,
         'refereeverificationstatus' => $record->refereeverificationstatus()->name,
