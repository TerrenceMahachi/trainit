...
use App\Controllers\RosterskillsController;
use App\Helpers\ViewAccess;
use App\Models\Rosterskill;

$uri = 'rosterskills';
$table = 'rosterskill';
$options['view'] = 'rosterskills';
$options['title'] = 'Rosterskill';
$options['MainController'] = new RosterskillsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Rosterskill::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'servicefunction' => $record->servicefunction()->name,
         'skillitem' => $record->skillitem()->name,
         'proficiencylevel' => $record->proficiencylevel()->name
        ];
    }, $ds);
