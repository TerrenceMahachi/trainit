...
use App\Controllers\RosterstatuseventsController;
use App\Helpers\ViewAccess;
use App\Models\Rosterstatusevent;

$uri = 'rosterstatusevents';
$table = 'rosterstatusevent';
$options['view'] = 'rosterstatusevents';
$options['title'] = 'Rosterstatusevent';
$options['MainController'] = new RosterstatuseventsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Rosterstatusevent::where('iD', $recordiD);
    if (count($records) === 0) {
