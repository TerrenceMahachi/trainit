...
use App\Controllers\RosterworkhistorysController;
use App\Helpers\ViewAccess;
use App\Models\Rosterworkhistory;

$uri = 'rosterworkhistorys';
$table = 'rosterworkhistory';
$options['view'] = 'rosterworkhistorys';
$options['title'] = 'Rosterworkhistory';
$options['MainController'] = new RosterworkhistorysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Rosterworkhistory::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'sectortype' => $record->sectortype()->name,
         'engagementbasis' => $record->engagementbasis()->name,
