...
use App\Controllers\ServicefunctionsController;
use App\Helpers\ViewAccess;
use App\Models\Servicefunction;

$uri = 'servicefunctions';
$table = 'servicefunction';
$options['view'] = 'servicefunctions';
$options['title'] = 'Servicefunction';
$options['MainController'] = new ServicefunctionsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Servicefunction::where('iD', $recordiD);
    if (count($records) === 0) {
