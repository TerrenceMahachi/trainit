...
use App\Controllers\ServicelevelpolicysController;
use App\Helpers\ViewAccess;
use App\Models\Servicelevelpolicy;

$uri = 'servicelevelpolicys';
$table = 'servicelevelpolicy';
$options['view'] = 'servicelevelpolicys';
$options['title'] = 'Servicelevelpolicy';
$options['MainController'] = new ServicelevelpolicysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Servicelevelpolicy::where('iD', $recordiD);
    if (count($records) === 0) {
