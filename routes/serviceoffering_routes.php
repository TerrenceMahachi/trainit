...
use App\Controllers\ServiceofferingsController;
use App\Helpers\ViewAccess;
use App\Models\Serviceoffering;

$uri = 'serviceofferings';
$table = 'serviceoffering';
$options['view'] = 'serviceofferings';
$options['title'] = 'Serviceoffering';
$options['MainController'] = new ServiceofferingsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Serviceoffering::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'servicecategory' => $record->servicecategory()->name,
         'code' => $record->code,
         'name' => $record->name,
