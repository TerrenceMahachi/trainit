...
use App\Controllers\ServicerequestattachmentsController;
use App\Helpers\ViewAccess;
use App\Models\Servicerequestattachment;

$uri = 'servicerequestattachments';
$table = 'servicerequestattachment';
$options['view'] = 'servicerequestattachments';
$options['title'] = 'Servicerequestattachment';
$options['MainController'] = new ServicerequestattachmentsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Servicerequestattachment::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'messagevisibility' => $record->messagevisibility()->name
        ];
    }, $ds);
