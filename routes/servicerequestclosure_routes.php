...
use App\Controllers\ServicerequestclosuresController;
use App\Helpers\ViewAccess;
use App\Models\Servicerequestclosure;

$uri = 'servicerequestclosures';
$table = 'servicerequestclosure';
$options['view'] = 'servicerequestclosures';
$options['title'] = 'Servicerequestclosure';
$options['MainController'] = new ServicerequestclosuresController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Servicerequestclosure::where('iD', $recordiD);
    if (count($records) === 0) {
