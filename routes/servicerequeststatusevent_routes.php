...
use App\Controllers\ServicerequeststatuseventsController;
use App\Helpers\ViewAccess;
use App\Models\Servicerequeststatusevent;

$uri = 'servicerequeststatusevents';
$table = 'servicerequeststatusevent';
$options['view'] = 'servicerequeststatusevents';
$options['title'] = 'Servicerequeststatusevent';
$options['MainController'] = new ServicerequeststatuseventsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Servicerequeststatusevent::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'servicerequest' => $record->servicerequest()->name,
         'servicerequeststatus' => $record->servicerequeststatus()->name,
         'notes' => $record->notes
        ];
    }, $ds);
