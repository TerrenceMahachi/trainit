...
use App\Controllers\ServicerequesttriagesController;
use App\Helpers\ViewAccess;
use App\Models\Servicerequesttriage;

$uri = 'servicerequesttriages';
$table = 'servicerequesttriage';
$options['view'] = 'servicerequesttriages';
$options['title'] = 'Servicerequesttriage';
$options['MainController'] = new ServicerequesttriagesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Servicerequesttriage::where('iD', $recordiD);
    if (count($records) === 0) {
