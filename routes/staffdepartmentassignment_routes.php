...
use App\Controllers\StaffdepartmentassignmentsController;
use App\Helpers\ViewAccess;
use App\Models\Staffdepartmentassignment;

$uri = 'staffdepartmentassignments';
$table = 'staffdepartmentassignment';
$options['view'] = 'staffdepartmentassignments';
$options['title'] = 'Staffdepartmentassignment';
$options['MainController'] = new StaffdepartmentassignmentsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Staffdepartmentassignment::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'staffprofile' => $record->staffprofile()->name,
