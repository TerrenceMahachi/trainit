...
use App\Controllers\StaffleaveapprovalsController;
use App\Helpers\ViewAccess;
use App\Models\Staffleaveapproval;

$uri = 'staffleaveapprovals';
$table = 'staffleaveapproval';
$options['view'] = 'staffleaveapprovals';
$options['title'] = 'Staffleaveapproval';
$options['MainController'] = new StaffleaveapprovalsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Staffleaveapproval::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'staffleave' => $record->staffleave()->name,
         'leavestatus' => $record->leavestatus()->name,
