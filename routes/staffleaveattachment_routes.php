...
use App\Controllers\StaffleaveattachmentsController;
use App\Helpers\ViewAccess;
use App\Models\Staffleaveattachment;

$uri = 'staffleaveattachments';
$table = 'staffleaveattachment';
$options['view'] = 'staffleaveattachments';
$options['title'] = 'Staffleaveattachment';
$options['MainController'] = new StaffleaveattachmentsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Staffleaveattachment::where('iD', $recordiD);
    if (count($records) === 0) {
