...
use App\Controllers\StafftimeapprovalsController;
use App\Helpers\ViewAccess;
use App\Models\Stafftimeapproval;

$uri = 'stafftimeapprovals';
$table = 'stafftimeapproval';
$options['view'] = 'stafftimeapprovals';
$options['title'] = 'Stafftimeapproval';
$options['MainController'] = new StafftimeapprovalsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Stafftimeapproval::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'stafftimeentry' => $record->stafftimeentry()->name,
