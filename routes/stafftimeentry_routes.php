...
use App\Controllers\StafftimeentrysController;
use App\Helpers\ViewAccess;
use App\Models\Stafftimeentry;

$uri = 'stafftimeentrys';
$table = 'stafftimeentry';
$options['view'] = 'stafftimeentrys';
$options['title'] = 'Stafftimeentry';
$options['MainController'] = new StafftimeentrysController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Stafftimeentry::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'activitycategory' => $record->activitycategory()->name,
