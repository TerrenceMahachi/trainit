...
use App\Controllers\StatutoryreturnsController;
use App\Helpers\ViewAccess;
use App\Models\Statutoryreturn;

$uri = 'statutoryreturns';
$table = 'statutoryreturn';
$options['view'] = 'statutoryreturns';
$options['title'] = 'Statutoryreturn';
$options['MainController'] = new StatutoryreturnsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Statutoryreturn::where('iD', $recordiD);
    if (count($records) === 0) {
