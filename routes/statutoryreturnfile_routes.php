...
use App\Controllers\StatutoryreturnfilesController;
use App\Helpers\ViewAccess;
use App\Models\Statutoryreturnfile;

$uri = 'statutoryreturnfiles';
$table = 'statutoryreturnfile';
$options['view'] = 'statutoryreturnfiles';
$options['title'] = 'Statutoryreturnfile';
$options['MainController'] = new StatutoryreturnfilesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Statutoryreturnfile::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'statutoryreturn' => $record->statutoryreturn()->name,
