...
use App\Controllers\VettingrecommendationsController;
use App\Helpers\ViewAccess;
use App\Models\Vettingrecommendation;

$uri = 'vettingrecommendations';
$table = 'vettingrecommendation';
$options['view'] = 'vettingrecommendations';
$options['title'] = 'Vettingrecommendation';
$options['MainController'] = new VettingrecommendationsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Vettingrecommendation::where('iD', $recordiD);
    if (count($records) === 0) {
