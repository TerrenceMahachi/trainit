...
use App\Controllers\WorkassignmentsController;
use App\Helpers\ViewAccess;
use App\Models\Workassignment;

$uri = 'workassignments';
$table = 'workassignment';
$options['view'] = 'workassignments';
$options['title'] = 'Workassignment';
$options['MainController'] = new WorkassignmentsController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Workassignment::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'assigned_role' => $record->assigned_role()->name,
         'rate_currency' => $record->rate_currency,
