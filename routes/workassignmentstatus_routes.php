...
use App\Controllers\WorkassignmentstatussController;
use App\Helpers\ViewAccess;
use App\Models\Workassignmentstatus;

$uri = 'workassignmentstatuss';
$table = 'workassignmentstatus';
$options['view'] = 'workassignmentstatuss';
$options['title'] = 'Workassignmentstatus';
$options['MainController'] = new WorkassignmentstatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Workassignmentstatus::where('iD', $recordiD);
    if (count($records) === 0) {
...
$router->addRoute('POST', "/get-$table-records", function () use ($router, $options) {
    ...
    $data['records'] = array_map(function ($record) {
        return [
         ...
         'assignmentstatus' => $record->assignmentstatus()->name,
         'notes' => $record->notes
        ];
    }, $ds);
