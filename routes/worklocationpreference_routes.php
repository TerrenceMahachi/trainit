...
use App\Controllers\WorklocationpreferencesController;
use App\Helpers\ViewAccess;
use App\Models\Worklocationpreference;

$uri = 'worklocationpreferences';
$table = 'worklocationpreference';
$options['view'] = 'worklocationpreferences';
$options['title'] = 'Worklocationpreference';
$options['MainController'] = new WorklocationpreferencesController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Worklocationpreference::where('iD', $recordiD);
    if (count($records) === 0) {
