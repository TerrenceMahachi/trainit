...
use App\Controllers\WorkrightstatussController;
use App\Helpers\ViewAccess;
use App\Models\Workrightstatus;

$uri = 'workrightstatuss';
$table = 'workrightstatus';
$options['view'] = 'workrightstatuss';
$options['title'] = 'Workrightstatus';
$options['MainController'] = new WorkrightstatussController();

/** Fetch a record by id or render the 404 page (used by view/edit routes). */
$options['findOr404'] = function ($recordiD) use ($options) {
    $records = Workrightstatus::where('iD', $recordiD);
    if (count($records) === 0) {
