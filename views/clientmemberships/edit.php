    ...
    <section class="py-4">
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="update_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Clientmemberrole: </label>
                    <select class="form-select form-select-lg f-sel" name="clientmemberrole" required>
                        <option value="">Select Clientmemberrole</option>
                        <?php foreach (Clientmemberrole::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->clientmemberrole == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
