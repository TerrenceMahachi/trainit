                ...
                <div class="form-group mb-4 col-md-12">
                    ...
                    <select class="form-select form-select-lg f-sel" name="payslip" required>
                        ...
                        <?php foreach (Payslip::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->payslip == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> payment_method: </label>
                    <input type="text" name="payment_method" class="form-control form-control-lg" value="<?= htmlspecialchars($item->payment_method ?? '', ENT_QUOTES) ?>" />
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> transaction_reference: </label>
                    <input type="text" name="transaction_reference" class="form-control form-control-lg" value="<?= htmlspecialchars($item->transaction_reference ?? '', ENT_QUOTES) ?>" />
                </div>
