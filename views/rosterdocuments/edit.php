    ...
    <section class="py-4">
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="update_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Documenttype: </label>
                    <select class="form-select form-select-lg f-sel" name="documenttype" required>
                        <option value="">Select Documenttype</option>
                        <?php foreach (Documenttype::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->documenttype == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> original_name: </label>
                    <input type="text" name="original_name" class="form-control form-control-lg" value="<?= htmlspecialchars($item->original_name ?? '', ENT_QUOTES) ?>" />
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> file_size_kb: </label>
                    <input type="number" name="file_size_kb" class="form-control form-control-lg" value="<?= htmlspecialchars($item->file_size_kb ?? '', ENT_QUOTES) ?>" />
                </div>
