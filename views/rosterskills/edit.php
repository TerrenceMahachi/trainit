    ...
    <section class="py-4">
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="update_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Servicefunction: </label>
                    <select class="form-select form-select-lg f-sel" name="servicefunction" required>
                        <option value="">Select Servicefunction</option>
                        <?php foreach (Servicefunction::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->servicefunction == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Skillitem: </label>
                    <select class="form-select form-select-lg f-sel" name="skillitem" required>
                        <option value="">Select Skillitem</option>
                        <?php foreach (Skillitem::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->skillitem == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Proficiencylevel: </label>
                    <select class="form-select form-select-lg f-sel" name="proficiencylevel" required>
                        <option value="">Select Proficiencylevel</option>
                        <?php foreach (Proficiencylevel::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->proficiencylevel == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
