    ...
    <section class="py-4">
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="update_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Servicerequeststatus: </label>
                    <select class="form-select form-select-lg f-sel" name="servicerequeststatus" required>
                        <option value="">Select Servicerequeststatus</option>
                        <?php foreach (Servicerequeststatus::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->servicerequeststatus == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
