                ...
                <div class="form-group mb-4 col-md-12">
                    ...
                    <select class="form-select form-select-lg f-sel" name="leavestatus" required>
                        ...
                        <?php foreach (Leavestatus::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->leavestatus == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> decision_notes: </label>
                    <input type="text" name="decision_notes" class="form-control form-control-lg" value="<?= htmlspecialchars($item->decision_notes ?? '', ENT_QUOTES) ?>" />
                </div>
