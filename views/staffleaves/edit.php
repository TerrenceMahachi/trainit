    ...
    <section class="py-4">
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="update_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Leavetype: </label>
                    <select class="form-select form-select-lg f-sel" name="leavetype" required>
                        <option value="">Select Leavetype</option>
                        <?php foreach (Leavetype::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->leavetype == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> days_requested: </label>
                    <input type="number" name="days_requested" class="form-control form-control-lg" value="<?= htmlspecialchars($item->days_requested ?? '', ENT_QUOTES) ?>" />
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> reason: </label>
                    <input type="text" name="reason" class="form-control form-control-lg" value="<?= htmlspecialchars($item->reason ?? '', ENT_QUOTES) ?>" />
                </div>
