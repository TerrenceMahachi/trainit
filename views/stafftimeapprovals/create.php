        ...
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="create_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Stafftimeentry: </label>
                    <select class="form-select form-select-lg f-sel" name="stafftimeentry" required>
                        <option value="">Select Stafftimeentry</option>
                        <?php foreach (Stafftimeentry::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>"><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> is_approved: </label>
                    <input type="text" name="is_approved" class="form-control form-control-lg" />
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> review_notes: </label>
                    <input type="text" name="review_notes" class="form-control form-control-lg" />
                </div>
                </div>
