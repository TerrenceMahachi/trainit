    ...
    <section class="py-4">
        <div class="container rounded-4 p-4 bg-white shadow-sm">
            ...
            <form class="form py-3 m-md-3" id="update_item_form">
                ...
                <div class="form-group mb-4 col-md-12">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> Activitycategory: </label>
                    <select class="form-select form-select-lg f-sel" name="activitycategory" required>
                        <option value="">Select Activitycategory</option>
                        <?php foreach (Activitycategory::all() as $selector): ?>
                            <option value="<?= $selector->iD; ?>" <?= $data['record']->activitycategory == $selector->iD ? 'selected' : ''; ?>><?= htmlspecialchars($selector->name ?? ''); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> work_date: </label>
                    <input type="date" name="work_date" class="form-control form-control-lg" value="<?= htmlspecialchars($item->work_date ?? '', ENT_QUOTES) ?>" />
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> hours: </label>
                    <input type="number" name="hours" class="form-control form-control-lg" value="<?= htmlspecialchars($item->hours ?? '', ENT_QUOTES) ?>" />
                </div>
                <div class="form-group col-md-12 mb-4">
                    <label class="text-muted fw-lighter fs-6 mb-3 text-muted"> task_summary: </label>
                    <input type="text" name="task_summary" class="form-control form-control-lg" value="<?= htmlspecialchars($item->task_summary ?? '', ENT_QUOTES) ?>" />
                </div>
